...
from strengths_methods import strengths_attacks_extended_all
from strengths_methods import strengths_weights_extended_all
from strengths_methods import strengths_nopref_all
...
def extract_sentences_for_topic_entities(director, writer, actors, lower_case_reviews_sents, film_title):
	...
	topic_entities_dict["director"]["entities"][director] = dict()
	topic_entities_dict["director"]["entities"][director]["args"] = []

	topic_entities_dict["writer"]["entities"] = dict()
	
	topic_entities_dict["writer"]["entities"][writer] = dict()
	topic_entities_dict["writer"]["entities"][writer]["args"] = []
	...
	for review in lower_case_reviews_sents:	
		for sent in review:
			...
			if len(sent) > 25:
				...
				if neutrality <= 0.9:
					ent_split = director.lower().replace("'s", " ").split()
					if len(ent_split) > 1:
					...
					if ent_appearance:
						...
						topic_entities_dict = decide_arg_vote_entity(topic_entities_dict, "director", director, sent, polarity)
					...
					if writer not in director:
						ent_split = writer.lower().replace("'s", " ").split()
						...
						ent_appearance = any([find_whole_word(x)(sent) is not None for x in ent_split])
						if ent_appearance:
							topic_entities_dict = decide_arg_vote_entity(topic_entities_dict, "writer", writer, sent, polarity)
...
def process_url_pref(url):
	...
	print("The following url has been received and we search for the AF file "+url)
	...
	if nr_critics > 0:
		...
		if (rt_score == None):
			print("Rt score is not available yet")
		...
		if not os.path.isfile(af_file):
			print("file not found (AF FILE). Creating a new one")
...
def readAFfileExtended(topic_entities_dict, nr_critics, semantics):
	...
	booleanPref= False
	numberReviewOrCritic = input("Do you want to divide by nr_critics or nr_reviews? ")
	...
	if(answerPreferences == "y"):
		...
		print(listAnsPref)
		#scegliamo se dividere per nr_critics or nr_reviews
		...
		if all(value == '' for value in listAnsPref.values()):
			booleanPref = False
			if semantics in ["dfquad", "quad", "euler", "energy"]:
					film_score = strengths_nopref_all(semantics, topic_entities_dict,nr_critics, numberReviewOrCritic)
			else:
				
				film_score_dfquad =strengths_nopref_all("dfquad", topic_entities_dict,nr_critics, numberReviewOrCritic)
				
				film_score_quad = strengths_nopref_all("quad", topic_entities_dict,nr_critics, numberReviewOrCritic)

				film_score_euler = strengths_nopref_all("euler", topic_entities_dict,nr_critics, numberReviewOrCritic)

				film_score_energy = strengths_nopref_all("energy", topic_entities_dict,nr_critics, numberReviewOrCritic)
			...
			if(preferenceMethod == 'w'):
				if semantics in ["dfquad", "quad", "euler", "energy"]:
					film_score = strengths_weights_extended_all(semantics, topic_entities_dict,nr_critics, listAnsPref, list_keys, list_entities, list_entities_complete, numberReviewOrCritic)
				else:
					
					film_score_dfquad = strengths_weights_extended_all("dfquad", topic_entities_dict,nr_critics, listAnsPref, list_keys, list_entities, list_entities_complete, numberReviewOrCritic)
					listAnsPref =copy.deepcopy(listAnsPrefOrig)
					
					film_score_quad = strengths_weights_extended_all("quad", topic_entities_dict,nr_critics, listAnsPref, list_keys, list_entities,list_entities_complete,numberReviewOrCritic)
					listAnsPref =copy.deepcopy(listAnsPrefOrig)

					film_score_euler = strengths_weights_extended_all("euler", topic_entities_dict,nr_critics, listAnsPref, list_keys, list_entities, list_entities_complete,numberReviewOrCritic)
					listAnsPref =copy.deepcopy(listAnsPrefOrig)

					film_score_energy = strengths_weights_extended_all("energy", topic_entities_dict,nr_critics, listAnsPref, list_keys, list_entities, list_entities_complete,numberReviewOrCritic)
					listAnsPref =copy.deepcopy(listAnsPrefOrig)
					film_score = [film_score_dfquad, film_score_quad, film_score_euler, film_score_energy]

			elif(preferenceMethod == 'a'):
				if semantics in ["dfquad", "quad", "euler", "energy"]:
					film_score = strengths_attacks_extended_all(semantics, topic_entities_dict,nr_critics, listAnsPref, list_keys, list_entities,numberReviewOrCritic)
				else:
					
					film_score_dfquad = strengths_attacks_extended_all("dfquad", topic_entities_dict,nr_critics, listAnsPref, list_keys, list_entities,numberReviewOrCritic)
					listAnsPref =copy.deepcopy(listAnsPrefOrig)
					
					film_score_quad = strengths_attacks_extended_all("quad", topic_entities_dict,nr_critics, listAnsPref, list_keys, list_entities,numberReviewOrCritic)
					listAnsPref =copy.deepcopy(listAnsPrefOrig)

					film_score_euler = strengths_attacks_extended_all("euler", topic_entities_dict,nr_critics, listAnsPref, list_keys, list_entities,numberReviewOrCritic)
					listAnsPref =copy.deepcopy(listAnsPrefOrig)

					film_score_energy = strengths_attacks_extended_all("energy", topic_entities_dict,nr_critics, listAnsPref, list_keys, list_entities,numberReviewOrCritic)
					listAnsPref =copy.deepcopy(listAnsPrefOrig)
					film_score = [film_score_dfquad, film_score_quad, film_score_euler, film_score_energy]
	...
	return film_score, booleanPref, numberReviewOrCritic
...
def useUrl(url, method, semantics):
	...
	if method == "sent":
		...
		if (film_title == rt_score == topic_entities_dict == nr_critics == None):
			print("Sorry, no enough info is available now on RT website")
		else:
			#here we read the AF file and we ask the user if he/she wants to add preferences on the BASIS of the AF file structure
			#numberReviewOrCritic = "nr_reviews"
			film_score, booleanPref, numberReviewOrCritic = readAFfileExtended(topic_entities_dict, nr_critics, semantics)
			#here we retrieve the final results: rt score, strength score, film title
			results.append((rt_score, film_score, film_title, numberReviewOrCritic))
	

			#if film_score is not a list, then you have called just one semantic and you save it in the related file. Otherwise you have called all the available semantics and you save it in another file
...
if __name__ == "__main__":
	def analyseToy():
		...
		film_score, booleanPref, numberReviewsOrCritic = readAFfileExtended(topic_entities_dict, nr_critics, semantics)
		results.append(("RT score", film_score, "Toy Example 24", numberReviewsOrCritic))
		if not isinstance(film_score, list):
			if booleanPref:
	...
	with_themes="n"
	semantics = input("Which semantics? dfquad/quad/euler/energy ")
	#method = input("sent/nlp? ")
	'''
	The original code provides the user with the possibility to choose the nlp method, however the original code doesn't seem to work. So for the moment we put sentiment analysis as default.
	'''
	method = "sent"
	booleanToy = input("Do you want to analyse the toy example or a real movie? t/m ")
	if booleanToy.lower() == "t":
		analyseToy()
	elif booleanToy.lower()=="m":
		analyseRealMovie()
