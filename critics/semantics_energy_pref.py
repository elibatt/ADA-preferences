...
def quadratic_energy_strength(base_score, att, supp):
	energyValue = energy_value(att,supp)
	weigth = base_score
	return weigth + (1-weigth) * hFunction(energyValue) -weigth*hFunction(energyValue*-1)



def strengths_energy_weights_extended(topic_entities_dict, nr_critics, listAnsPref, list_keys, list_entities_preferences, list_entities_complete):
	nr_votes = dict()
	levelsLists=[]
	print("Sono in ENERGY WEIGHTS")
	...
	for key in topic_entities_dict.keys():
		...
		if key == "writer" or key == "director":
			...
			feature_strengths[key] = quadratic_energy_strength(base_score_acting_weighted, atts,supps)
			...
			feature_strengths[key] = quadratic_energy_strength(base_score_themes_weighted, atts, supps)
	...
	feature_strengths["film"] = quadratic_energy_strength(film_base_score, m_atts,m_supps)
...
def strengths_energy_attacks_extended(topic_entities_dict, nr_critics, listAnsPref, list_keys, list_entities_preferences, list_entities_complete):
	print("Sono in ENERGY ATTACKS, nr critics %d" % nr_critics)
		...
		if entity in atts_btw_entities_children.keys():
			...
			for (childEntity, lista) in	atts_btw_entities_children[entity]:
				...
				if len(lista)==0:
					...
					feature_strengths[entity+"_"+childEntity]=quadratic_energy_strength(baseScoreChildEntity[childEntity], attsStrenghts, [])
	...
	for (key, lista) in atts_between_keys:
		...
		if len(lista)==0:
			if (len(entityAttackers[key])==0 and len(entitySupporters[key])==0):
				...
				feature_strengths[key] =  quadratic_energy_strength(baseScoreEntity[key], entityAttackers[key],entitySupporters[key])
		...
		#controllo attacchi verticali e orizzontali (dx sx, basso)
			...
			feature_strengths[key] =  quadratic_energy_strength(baseScoreEntity[key], unifiedAttackers, entitySupporters[key])
