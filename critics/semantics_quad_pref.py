...
def strengths_quad_weights_extended(topic_entities_dict, nr_critics, listAnsPref, list_keys, list_entities_preferences, list_entities_complete):
	nr_votes = dict()
	levelsLists=[]
	print("Sono in QUAD WEIGHTS")
	...
	for key in topic_entities_dict.keys():
		...
		if key == "writer" or key == "director":
			...
			feature_strengths[key] = combination_function(base_score_acting_weighted, strength_aggregation_att(base_score_acting_weighted,atts), strength_aggregation_sup(base_score_acting_weighted,supps))
			...
			feature_strengths[key] = combination_function(base_score_themes_weighted, strength_aggregation_att(base_score_themes_weighted,atts), strength_aggregation_sup(base_score_themes_weighted,supps))
...
def strengths_quad_attacks_extended(topic_entities_dict, nr_critics, listAnsPref, list_keys, list_entities_preferences, list_entities_complete):
	print("Sono in QUAD ATTACKS, nr critics %d" % nr_critics)
		...
		if entity in atts_btw_entities_children.keys():
			...
			for (childEntity, lista) in	atts_btw_entities_children[entity]:
				...
				if len(lista)==0:
					...
					feature_strengths[entity+"_"+childEntity]=combination_function(baseScoreChildEntity[childEntity], strength_aggregation_att(baseScoreChildEntity[childEntity],attsStrenghts), strength_aggregation_sup(baseScoreChildEntity[childEntity],[]))
	...
	for (key, lista) in atts_between_keys:
		...
		if len(lista)==0:
			if (len(entityAttackers[key])==0 and len(entitySupporters[key])==0):
				...
				feature_strengths[key] =  combination_function(baseScoreEntity[key], strength_aggregation_att(baseScoreEntity[key],entityAttackers[key]), strength_aggregation_sup(baseScoreEntity[key],entitySupporters[key]))
		...
		#controllo attacchi verticali e orizzontali (dx sx, basso)
			...
			feature_strengths[key] =  combination_function(baseScoreEntity[key], strength_aggregation_att(baseScoreEntity[key],unifiedAttackers), strength_aggregation_sup(baseScoreEntity[key],entitySupporters[key]))
...
if __name__ == "__main__":
	...
	film_score = strengths_quad_pref(topic_entities_dict, nr_critics)
	print(film_score)