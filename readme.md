...
3) At this point, the code assigns each extracted sentence to a certain key or entity. This is followed by  an _augmentation_ phase that recursively copies the entities' reviews up to the parent key, and finally it also augments all the reviews inside the keys _director_, _writer_, _acting_ into the key _film_.
